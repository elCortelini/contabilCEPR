    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 glass-card p-6 rounded-2xl">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Cadastre parceiros comerciais e efetue pagamentos diretos com débito na carteira</p>
        </div>

        <div className="flex items-center gap-4">
          <div className="text-right">
            <span className="text-xs text-slate-500 font-medium">Pendente Total a Pagar</span>
            <p className="text-xl font-black text-amber-600">{formatBrl(totalPendente)}</p>
          </div>
          <button
            ...
            <div key={f.id} className="glass-card p-6 rounded-2xl flex flex-col justify-between space-y-4">
              <div>
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-bold text-base text-slate-900">{f.nome}</h3>
                    {f.contato && (
                      <p className="text-xs text-slate-500 flex items-center gap-1 mt-1">
                        <Phone className="w-3.5 h-3.5 text-indigo-600" /> {f.contato}
                      </p>
                    )}
                    {f.endereco && (
                      <p className="text-xs text-slate-500 flex items-center gap-1 mt-0.5">
                        <MapPin className="w-3.5 h-3.5 text-slate-400" /> {f.endereco}
                      </p>
                    )}
                  </div>
                  <div className="text-right">
                    <span className="text-[10px] text-slate-500 uppercase font-semibold">Pendente</span>
                    <p className={`text-base font-black ${f.saldoPendente > 0 ? 'text-amber-600' : 'text-emerald-600'}`}>
                ...
                {f.observacoes && (
                  <p className="text-xs text-slate-600 bg-slate-50 p-2.5 rounded-xl border border-slate-200 mt-3">
                    {f.observacoes}
                  </p>
                )}
              </div>
              ...
              <div className="pt-3 border-t border-slate-200">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-xs transition cursor-pointer shadow-md shadow-emerald-600/30"
                >
          ...
          <div className="bg-white w-full max-w-md p-6 rounded-2xl space-y-4 border border-slate-200 shadow-2xl text-slate-800">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">Saldo Pendente Inicial</label>
                  <input
                    type="number"
                    step="0.01"
                    placeholder="0.00"
                    ...
                    className="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2 text-sm text-slate-800 focus:outline-none focus:border-indigo-600 font-semibold text-amber-600"
                  />
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md p-6 rounded-2xl space-y-4 border border-slate-200 shadow-2xl text-slate-800">
            ...
            <form onSubmit={handlePaySubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Carteira de Origem (Débito)</label>
                <select
                  required
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Valor a Pagar (R$)</label>
                <input
                  type="number"
                  step="0.01"
                  required
